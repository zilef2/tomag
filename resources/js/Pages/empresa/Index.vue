...
import Delete from '@/Pages/empresa/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("empresa.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.empresas?.data.forEach((empresa) => {
            data.selectedId.push(empresa.id)
        })
    }
}
const select = () => {
...
<template>
    ...
    <AuthenticatedLayout>
        <Breadcrumb :title="title" :breadcrumbs="breadcrumbs" class="capitalize text-2xl font-bold"/>
        <div class="space-y-4">
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    ...
                    <PrimaryButton class="rounded-none" @click="data.createOpen = true"
                        v-if="can(['create empresa'])"
                    >
                        {{ lang().button.add }}
                    </PrimaryButton>

                    <Create v-if="can(['create empresa'])" :numberPermissions="props.numberPermissions"
                        :titulos="titulos" :show="data.createOpen" @close="data.createOpen = false" :title="props.title"
                        :losSelect=props.losSelect />

                    <Edit v-if="can(['update empresa'])" :titulos="titulos"
                        :numberPermissions="props.numberPermissions" :show="data.editOpen" @close="data.editOpen = false"
                        :empresaa="data.empresao" :title="props.title" :losSelect=props.losSelect />

                    <Delete v-if="can(['delete empresa'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :empresaa="data.empresao"
                        :title="props.title" />
                </div>
            </div>
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['delete empresa'])" class="px-3 py-1.5"
                        ...
                        <tbody>
                            <tr v-for="(claseFromController, indexu) in props.fromController.data" :key="indexu"
                                    ...
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update user'])" type="button"
                                                @click="(data.editOpen = true), (data.empresao = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['delete user'])" type="button"
                                                @click="(data.deleteOpen = true), (data.empresao = claseFromController)"
                                ...
                                <td v-for="titulo in titulos" class="whitespace-nowrap py-4 px-2 sm:py-3">
                                    ...
                                    <span v-if="titulo['type'] === 'foreign'"> {{ claseempresaa[titulo['order']][titulo['nameid']] }} </span>
                                </td>
