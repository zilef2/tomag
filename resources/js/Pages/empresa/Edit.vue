...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.empresaa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.empresaa[names['order']]
        });

        // form.codigo = props.empresaa?.codigo
    }
})

const update = () => {
    form.put(route('empresa.update', props.empresaa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.empresaa.empresa_id]"></v-select>
